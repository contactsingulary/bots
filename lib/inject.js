(function(window, document) {
  ...
  let hasConsent = false, showingConsent = false, sessionId = null, userId = null;
  ...
  // Helper function to generate UUID user ID
  ...
  function saveConsent(accepted, nonEssential = true) {
    if (accepted) {
      ...
      // Generate and store session ID and user ID
      sessionId = generateSessionId();
      userId = generateUserId();
      localStorage.setItem('singulary_session_id', sessionId);
      localStorage.setItem('singulary_user_id', userId);
      ...
      console.log('Consent accepted, session ID created:', sessionId, 'user ID created:', userId);
    } else {
      ...
      console.log('Consent revoked, session and user cleared');
    }
