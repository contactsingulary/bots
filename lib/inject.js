...
(function(window, document) {
  ...
  function hideChat() {
    ...
    // Save chat closed state to localStorage
  ...
  // Capture current scroll position from chat iframe
  function captureScrollPosition() {
    if (!chatFrame || !chatFrame.contentWindow || isTransitioning) return;
  ...
  // Restore saved scroll position in chat iframe
  function restoreScrollPosition() {
    if (!chatFrame || !chatFrame.contentWindow || isTransitioning) return;
    
    try {
      // Send saved scroll position to iframe
      chatFrame.contentWindow.postMessage({
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.origin === CONFIG.origin) {
      ...
      } else if (event.data?.type === 'scrollPosition' && typeof event.data.position === 'number') {
        // Save the scroll position for later restoration
        savedScrollPosition = event.data.position;
      }
    }
  });
