...
(function(window, document) {
  ...
  // Note: Removed unused scroll position variables
  ...
  function captureScrollPosition() {
    ...
    try {
      ...
      // Ignore errors when communicating with iframe
    }
  }
  ...
  function checkConsent() {
    // ✅ MIGRATION: Move old user_id to new web_user_id key
    const oldUserId = localStorage.getItem('singulary_user_id');
    if (oldUserId && !localStorage.getItem('singulary_web_user_id')) {
      localStorage.setItem('singulary_web_user_id', oldUserId);
    ...
    if (hasConsent) {
      ...
      userId = localStorage.getItem('singulary_web_user_id');
      ...
      if (!sessionId) {
        ...
        if (!userId) {
          ...
          localStorage.setItem('singulary_web_user_id', userId);
        }
    }
  ...
  function saveConsent(accepted, nonEssential = true) {
    if (accepted) {
      ...
      // ✅ CRITICAL FIX: Check if this is a fresh consent after rejection
      const wasRejected = !hasConsent && (sessionId === null || userId === null);
      
      if (wasRejected) {
        // ✅ Generate completely NEW session and user IDs for fresh start
        sessionId = generateSessionId();
        userId = generateUserId();
        localStorage.setItem('singulary_session_id', sessionId);
        localStorage.setItem('singulary_web_user_id', userId);
        ...
        if (!userId) {
          userId = localStorage.getItem('singulary_web_user_id') || generateUserId();
          localStorage.setItem('singulary_web_user_id', userId);
        }
      }
      ...
      localStorage.removeItem('singulary_web_user_id');
    ...
    if (!hasConsent && !showingConsent) {
      ...
      const sendConsentMessage = () => {
        if (chatFrame && chatFrame.contentWindow) {
          ...
          setTimeout(sendConsentMessage, 200);
        }
      };
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.origin === CONFIG.origin) {
      ...
      // Handle user rejecting consent in iframe
        ...
        // ✅ Enhanced reset sequence for clean state
        // First reset chat iframe to clean state
        if (chatFrame && chatFrame.contentWindow) {
          chatFrame.contentWindow.postMessage({
            type: 'resetChat'
          }, '*');
        ...
        // ✅ Wait for iframe reset, then close chat
        setTimeout(() => {
          ...
          if (window.searchBar) {
            ...
            if (window.toggleIcons) window.toggleIcons();  // Update send button visibility
          }
          if (window.searchIcon) {
            window.searchIcon.style.pointerEvents = '';
            window.searchIcon.style.opacity = '1';
        ...
        }, 150);  // ✅ Give iframe time to reset before closing
      ...
      // Handle request for pending search text after consent accepted
        ...
        if (pendingSearchText.trim() && chatFrame && chatFrame.contentWindow) {
          ...
          if (window.searchBar) {
            ...
            if (window.toggleIcons) window.toggleIcons();
          }
        }
      ...
      // Handle general URL navigation request from chat iframe (for links in messages)
      } else if (event.data?.type === 'navigateToUrl') {
        // Navigate parent window to any URL from chat links
        if (event.data.url && typeof event.data.url === 'string') {
          window.location.href = event.data.url;
        }
  ...
  function sendMobileStatus() {
    if (chatFrame && chatFrame.contentWindow) {
      ...
      const agentId = window.embedApp?.config?.agent_id || 'default';
  ...
  function renderWidget() {
    ...
    searchBar.addEventListener('focus', () => {
      if (!isOpen) {
        ...
        const storedUserId = localStorage.getItem('singulary_web_user_id');
      ...
      // On mobile, notify chat iframe to scroll to latest messages when input is focused
      if (CONFIG.isMobile && isOpen && chatFrame && chatFrame.contentWindow) {
        setTimeout(() => {
          chatFrame.contentWindow.postMessage({
            type: 'mobileInputFocus'
          }, '*');
        }, 200); // Delay to account for keyboard animation
      }
    });
  ...
  // Only initialize if we detect a valid agent_id
  function waitForValidAgentId() {
    const maxAttempts = 200; // 10 seconds max wait
    let attempts = 0;
    
    function checkForValidAgent() {
      attempts++;
      
      // Check if we have a valid agent_id (not default or undefined)
      const agentId = window.embedApp?.config?.agent_id;
      const hasValidAgent = agentId && agentId !== 'default' && agentId.length > 5;
      
             if (hasValidAgent) {
         console.log('✅ Singulary widget initialized with agent:', agentId);
         window.embedApp.init();
       } else if (attempts < maxAttempts) {
         // Reduce logging frequency - only log every 2 seconds
         if (attempts % 40 === 1) {
           console.log(`⏳ Waiting for agent_id... (${Math.floor(attempts/20)}s)`);
         }
        setTimeout(checkForValidAgent, 50);
      } else {
        console.warn('❌ No valid agent_id found after 10 seconds. Widget will not initialize.');
        console.warn('💡 Make sure to load the config script with a valid agent_id before inject.js');
        // Do NOT initialize without valid agent_id
      }
    }
  ...
  // Start looking for valid agent_id
  setTimeout(waitForValidAgentId, 0);
