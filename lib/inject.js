...
(function(window, document) {
  ...
  function waitForValidAgentId() {
    ...
    function checkForValidAgent() {
      ...
      if (hasValidAgent) {
        console.log('✅ Valid agent_id detected:', agentId);
        console.log('🚀 Initializing Singulary widget...');
        window.embedApp.init();
      } else if (attempts < maxAttempts) {
        if (attempts % 20 === 1) { // Log every second instead of every 50ms
          console.log(`⏳ Waiting for valid agent_id... attempt ${attempts}/${maxAttempts}`, {
            embedApp: !!window.embedApp,
            config: !!window.embedApp?.config,
            agent_id: agentId,
            isValid: hasValidAgent
          });
        }
