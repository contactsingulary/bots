...
(function(window, document) {
  ...
  function waitForConfigAndInit() {
    ...
    function checkForConfig() {
      ...
      if (window.embedApp && window.embedApp.config) {
        console.log('✅ Config found, initializing widget with agent_id:', window.embedApp.config.agent_id);
        window.embedApp.init();
      } else if (attempts < maxAttempts) {
        console.log(`⏳ Waiting for config... attempt ${attempts}/${maxAttempts}`);
        setTimeout(checkForConfig, 50);
      } else {
        console.warn('⚠️ Config timeout in inject.js, initializing with default');
        // Initialize embedApp if it doesn't exist
        if (!window.embedApp) {
          window.embedApp = {};
        }
        if (!window.embedApp.config) {
          window.embedApp.config = { agent_id: 'default' };
        }
        window.embedApp.init();
      }
    }
  ...
  // Start the config waiting process
  setTimeout(waitForConfigAndInit, 0);
