  ...
  function saveConsent(accepted, nonEssential = true) {
    if (accepted) {
      ...
      if (wasRejected) {
        ...
        localStorage.setItem('singulary_session_id', sessionId);
        localStorage.setItem('singulary_user_id', userId);
        console.log('🆕 Generated NEW session for fresh consent:', { sessionId, userId });
      } else {
        ...
        console.log('🔄 Using existing session for consent:', { sessionId, userId });
      }
    ...
    if (!CONFIG.isMobile) {
      ...
      sessionId = null;
      userId = null;
      console.log('❌ Cleared all session data for consent rejection');
    }
    ...
    if (chatFrame && chatFrame.contentWindow) {
      ...
      console.log('📤 Sent consent update to iframe:', { hasConsent, sessionId, userId });
    }
  }

  // Show consent modal when user needs to accept/reject privacy policy
  function showConsentIfNeeded() {
    console.log('🔍 Checking consent need:', { hasConsent, showingConsent });
    ...
    if (!hasConsent && !showingConsent) {
      ...
      const sendConsentMessage = () => {
        if (chatFrame && chatFrame.contentWindow) {
          console.log('✉️ Sending showConsent message to iframe');
          chatFrame.contentWindow.postMessage({
            type: 'showConsent'
          }, '*');
        } else {
          console.warn('⚠️ Chat iframe not ready, retrying...');
      ...
      console.log('ℹ️ Consent not needed:', { hasConsent, showingConsent });
    }
  }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.origin === CONFIG.origin) {
      ...
      // Handle user rejecting consent in iframe
        ...
        console.log('❌ User rejected consent - performing complete reset');
        ...
        setTimeout(() => {
          ...
          console.log('✅ Consent rejection complete - clean state restored');
